...
const DownloadPage: React.FC<DownloadPageProps> = ({ videoUrl, script, scriptV2, config, theme, onReset }) => {
    ...
    useEffect(() => {
        const handleGenerateMetadata = async () => {
            const activeScript = script || scriptV2;
            if (!activeScript || !theme) return;
            setIsMetadataLoading(true);
            setVideoMetadata(null);
            try {
                const fullScriptText = activeScript.map(s => s.narration).join('\n');
